...
NewtonExtended::NewtonExtended() : Fractal(FractalsType::NEWTON, nullptr),
                                   m_center(0.0, 0.0),
                                   m_offset(2.0),
...
NewtonExtended::NewtonExtended(const NewtonExtended& newtonExtended) : Fractal(FractalsType::NEWTON, nullptr),
                                                                       m_center(newtonExtended.m_center),
                                                                       m_offset(std::clamp(newtonExtended.m_offset, 1e-13, 2.)),
                                                                       m_mousePosition(nullptr)
{
	init();
}

NewtonExtended::NewtonExtended(const hgui::dvec2& center) : Fractal(FractalsType::NEWTON, nullptr),
...
NewtonExtended::NewtonExtended(const std::shared_ptr<Newton>& fractal) : Fractal(FractalsType::NEWTON, nullptr),
...
const hgui::dvec2& NewtonExtended::get_center() const
{
	return m_center;
}

double NewtonExtended::get_offset() const
{
	return m_offset;
}

void NewtonExtended::init()
{
	m_shader = hgui::ShaderManager::create(
		hgui::file_reader("assets/shaders/VertexShaderFractalExtended.glsl"),
		hgui::file_reader("assets/shaders/FragmentShaderNewtonExtended.glsl")
	);
