...
BurningShip::BurningShip(const hgui::vec2& center) :
	Fractal(FractalsType::BURNINGSHIP, nullptr),
...
BurningShip::BurningShip(const std::shared_ptr<BurningShipExtended>& fractal) :
	Fractal(FractalsType::BURNINGSHIP, nullptr),
...
const hgui::vec2& BurningShip::get_center() const
{
	return m_center;
}

float BurningShip::get_offset() const
{
	return m_offset;
}

void BurningShip::init()
{
	m_shader = hgui::ShaderManager::create(
		hgui::file_reader("assets/shaders/VertexShaderFractal.glsl"),
		hgui::file_reader("assets/shaders/FragmentShaderBurningShip.glsl")
	);
