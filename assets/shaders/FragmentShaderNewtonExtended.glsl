...
vec3 newton()
{
	...
	const dvec2 root[3] = {
		dvec2(1.0, 0.0),             // 1
		dvec2(-0.5, 0.86602540378),  // -0.5 + i*sqrt(3)/2
		dvec2(-0.5, -0.86602540378)  // -0.5 - i*sqrt(3)/2
	};
	double distance_to_root[3] = {
		length(root[0]),
		length(root[1]),
		length(root[2])
	};
	double distance_min = min(distance_to_root[0], min(distance_to_root[1], distance_to_root[2]));
	...
	{
		dvec2 squarred_complex = cmul(complex, complex);
		complex -= cdiv(cmul(squarred_complex, complex) - dvec2(1.0, 0.0), 3.0 * squarred_complex);
		for (int i = 0; i < 3; i++)
		{
	...
	{
		float continuous_iteration = float(iteration) - log2(log(max(float(distance_min), 10e-12)) / log(SMOOTH));
		switch (color)
		{
			case 0:
...
dvec2 cmul(dvec2 c1, dvec2 c2)
{
	return dvec2(c1.x * c2.x - c1.y * c2.y, c1.x * c2.y + c1.y * c2.x);
}

dvec2 cdiv(dvec2 c1, dvec2 c2)
{
	double d = c2.x * c2.x + c2.y * c2.y;
	return dvec2((c1.x * c2.x + c1.y * c2.y) / d, (c1.y * c2.x - c1.x * c2.y) / d);
}
